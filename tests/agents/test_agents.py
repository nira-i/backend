...
class TestNutritionAgent:
    @patch("nira_backend.agents.nutrition_agent.build_llm")
    def test_run_returns_string(
        self, mock_build: MagicMock, db: DatabaseConnection, tmp_path: Path
    ) -> None:
        from nira_backend.agents.nutrition_agent import NutritionAgent

        mock_build.return_value = _make_mock_llm("Meal logged!")
    ...
    def test_memory_is_saved_after_run(
        ...
        mock_build.return_value = _make_mock_llm("Got it!")
        agent = NutritionAgent(db=db, api_key="fake_key", data_dir=tmp_path)
        agent.run("Hello")
    ...
    def test_clear_memory_resets_count(
        ...
        mock_build.return_value = _make_mock_llm("OK")
        agent = NutritionAgent(db=db, api_key="fake_key", data_dir=tmp_path)
        agent.run("Test")
        agent.clear_memory()
...
class TestHealthAgent:
    @patch("nira_backend.agents.health_agent.build_llm")
    def test_run_returns_string(
        self, mock_build: MagicMock, db: DatabaseConnection, tmp_path: Path
    ) -> None:
        from nira_backend.agents.health_agent import HealthAgent

        mock_build.return_value = _make_mock_llm("Blood pressure logged.")
...
class TestExerciseAgent:
    @patch("nira_backend.agents.exercise_agent.build_llm")
    def test_run_returns_string(
        self, mock_build: MagicMock, db: DatabaseConnection, tmp_path: Path
    ) -> None:
        from nira_backend.agents.exercise_agent import ExerciseAgent

        mock_build.return_value = _make_mock_llm("Exercise logged!")
